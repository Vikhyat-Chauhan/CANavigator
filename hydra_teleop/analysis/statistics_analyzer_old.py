...
def run_analysis(
    ...
    CSV (case-insensitive accepted; REQUIRED):
      strategy | reached | elapse_time | event_violated | zone_violations | energy_kj | mean_power_kw
    CSV (OPTIONAL but supported if present):
      compute_energy_j   # CPU-side compute energy in joules (added by log transformer)
                         # If present, we report compute_energy_j_median in the summary.
    ...
    COL = {
        ...
        # Optional new column from transformer:
        "compute_energy_j": resolve_opt("compute_energy_j"),
    }
    ...
    # Build normalized dataframe; support missing compute_energy_j by filling NaN
    if COL["compute_energy_j"] is not None:
        compute_energy_series = to_num(df_raw[COL["compute_energy_j"]])
    else:
        compute_energy_series = pd.Series(np.nan, index=df_raw.index, dtype=float)
    ...
    df = pd.DataFrame({
        ...
        "_compute_energy_j": compute_energy_series,  # may be all-NaN if column absent
    })
    ...
    # If compute_energy_j is present (not all-NaN), add its median
