...
#!/usr/bin/env python3
# APE3: APE2 + Breadcrumbs + Safety shims + LiDAR-aware heading + NFZ repulsion + jerk/accel caps (+ optional tiny ML risk)
...
class SafetyCfg:
    ...
    dv_max_mps_per_s: float = 8.0    # base cap on linear speed change rate
    jw_max_radps2: float = 2.5       # cap on yaw-rate change per second

    # NEW: gentler accel when heading is “empty” (clear & aligned)
    ...
    yaw_align_rad: float = 0.25         # only slow accel if mostly aligned (|wz_cmd| <= this)
...
class RiskCfg:
    ...
    nofly_soft_w: float = 6.0       # weight for soft 1/(d+eps) cost
...
class LidarTargetNavigatorAPE3:
    """
    APE3 = APE2 + Breadcrumbs (tiny memory) + safety shims + LiDAR-aware heading + NFZ repulsion + accel caps.
    Public API and threading model remain the same as APE2.
    """
    def __init__(self,
                 teleop: GzTeleop,
                 cfg: TeleopConfig,
    ...
    def _choose_heading(self, scan: LaserScan, yaw_err: float, x: float, y: float) -> float:
        ...
        for deg in self._sweep_candidates(scan):
            ...
            nfz_term = nfz_soft0
            score = r - self._rc.center_weight*skew - self._rc.align_weight*abs(deg - yaw_err_deg) - self._rc.nofly_soft_w*nfz_term
            if score > best_score:
    ...
    def go_to(self,
        ...
        while True:
                ...
                if self._avoiding:
                    ...
                    if front < effective_safe_m:
                        ...
                        if self._crumb_hits_recent >= self._sc.crumb_oscillations_to_flip:
                            ...
                            try:
                                self._logger.info("[APE3] Breadcrumb: flip side-bias due to oscillation")
                            except Exception:
                                pass
            ...
            # Curvature-aware speed cap (slow down when |wz| is high)
            ...
            # ---------- Command ramp/jerk caps (adaptive accel on empty heading) ----------
